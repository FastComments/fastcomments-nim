...
proc to*(node: JsonNode, T: typedesc[UserSessionInfo]): UserSessionInfo =
  ...
  if node.kind == JObject:
    ...
    if node.hasKey("needsTOS") and node["needsTOS"].kind != JNull:
      result.needsTOS = some(to(node["needsTOS"], typeof(result.needsTOS.get())))
...
proc `%`*(obj: UserSessionInfo): JsonNode =
  ...
  if obj.needsTOS.isSome():
    result["needsTOS"] = %obj.needsTOS.get()
